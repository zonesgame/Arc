...
public class Lines{
    ...
    public static void line(float x, float y, float x2, float y2, float padding){
        line(x, y, x2, y2, CapStyle.square, padding);
    }
   ...
   // public static void circle(float x, float y, float rad, int vertices){
    ...
    // zones add begon
    private final static Vec2 node1 = new Vec2(), node2 = new Vec2(), node3 = new Vec2(), node4 = new Vec2();
    @Deprecated
    /** 菱形绘制*/
    public static void isometric(float tilex, float tiley, float tilew, float tileh){
...
//        Draw.color(Color.green);
        ...
        line((node1.x ) * scl, (node1.y ) * scl, (node2.x ) * scl, (node2.y ) * scl);
        line((node1.x ) * scl, (node1.y ) * scl, (node3.x ) * scl, (node3.y ) * scl);

        line((node4.x ) * scl, (node4.y ) * scl, (node2.x ) * scl, (node2.y ) * scl);
        line((node4.x ) * scl, (node4.y ) * scl, (node3.x ) * scl, (node3.y ) * scl);
    }

    /** 圆形绘制*/
    @Deprecated
    public static void _poly(float x, float y, int sides, float radiusx, float radiusy){
        floats.clear();
        for(int i = 0; i < sides; i++){
            float rad = (float)i / sides * Mathf.PI2;
            floats.add(Mathf.cos(rad) * radiusx + x, Mathf.sin(rad) * radiusy + y);
        }
    ...
    @Deprecated
    public static void _circle(float x, float y, float radx, float rady){
        poly(x, y, circleVertices(radx), rady);
    }
    // zones add end
}
